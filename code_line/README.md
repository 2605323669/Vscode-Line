...
- ![举例](img/11月27日排除目录A.png)
- ![举例](img/11月27日排除目录B.png)
...
- **功能扩展**：添加了统计指定类型文件的功能。
  ...
  - ![举例](img/12月13日修改前.png)
  - ![举例](img/12月13日修改后.png)
...
**功能开发**：
   - 在现有项目中添加了`countLines`函数，该函数通过调用Electron主进程中暴露的`calculateTotalLines`方法，获取指定文件或目录的代码行数统计信息。
   - `countLines`函数处理返回的数据，并将统计信息展示在三个不同的表格中：文件列表(`file-list`)、文件详细统计(`file-details-list`)和语言统计(`language-list`)。
   - 表格数据通过动态创建表格行(`<tr>`)并设置其内容来实现，确保界面能够根据用户选择的文件或目录实时更新。
   - 更新了选择文件和目录的按钮事件处理逻辑，确保在选择文件或目录后能够调用`countLines`函数并显示统计结果。
- ![举例](img/界面效果.gif)

## 12月25日

**重新发布为NPM包**：因为双重验证不通过的问题，重新发布为NPM包。
- ![举例](img/NPM包.png)
- ![举例](img/NPM下载.png)
- ![举例](img/NPM运行.png)
- ![举例](img/NPM结果.png)
...
**解决问题**：发布为NPM包之后，需要全局安装才能使用的问题，现在本地安装也可以进行使用。
